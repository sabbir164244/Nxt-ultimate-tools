...
export default function AIApplicationLetterGenerator() {
  const [userInput, setUserInput] = useState("Job Title: Software Engineer\nCompany Name: Tech Solutions Inc.\nMy key skills: React, Node.js, Team Leadership\nMy experience: 5 years in web development");
  ...
  const generatePrompt = (input: string, lang: string) => {
    return `Write a professional job application letter in ${lang} using Markdown. Details: ${input}. Structure it as a proper formal letter with sender's address, date, recipient's address, subject line, body paragraphs, and closing. Use formal language.`;
  };
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 border border-white/20">
          <div className="text-center mb-8">
            <div className="w-20 h-20 bg-gradient-to-r from-teal-500 to-cyan-600 rounded-full flex items-center justify-center mx-auto mb-4">
              <i className="ri-mail-send-line text-white text-3xl"></i>
            </div>
            <h2 className="text-3xl font-bold text-white mb-4">AI Application Letter Generator</h2>
          </div>
